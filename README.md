# PayDay Scheduler | Zaman Kilitli Transfer Otomasyonu

Trustless, on-chain time-locked payment scheduler built on the **ARC Network**. 
Users can deposit USDC (or any ERC20) and schedule future transfer instructions. The PayDay Backend Relayer automatically triggers and executes the transaction on-chain once the deadline is reached, paying the gas fee.
...
1. **🔒 Secure Deposit:** The user calls `scheduleOrder()`, locking their USDC safely in the smart contract. The user retains full ownership to **cancel and refund** their funds anytime prior to the execution date.
